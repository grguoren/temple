...
var PictureList = avalon.define('PictureList', function (vm) {
    vm.array = [];
    vm.changeList = [];
    vm.changeAll = false;
    vm.List_Page = 1;
    ...
    vm.getList = function (page) {
        ...
        $.ajax({
            type: 'POST',
            url: Config.WebUrl + 'Ajax_Service/GetPictureList',
    ...
    //}
    //-------------------------删除稱謂
    vm.DeletePictureFun = function (id) {
        if (window.confirm('你確定要刪除嗎？')) {
            $.ajax({
                type: 'POST',
                url: Config.WebUrl + 'Ajax_Service/DeletePicture',
                data: { id: id },
                timeout: 200000,
                success: function (ajaxData) {
                    if (ajaxData === "True") {
