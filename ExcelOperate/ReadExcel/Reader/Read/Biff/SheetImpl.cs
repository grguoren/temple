	...
	public class SheetImpl : Sheet
	{
		...
		internal SheetImpl(File f, SSTRecord sst, FormattingRecords fr, BOFRecord sb, BOFRecord wb, bool nf, WorkbookParser wp)
		{
			...
			while (bofs >= 1)
			{
                ...
                if (r.Code == ExcelOperate.Biff.Type.EOF.Value)
				{
					bofs--;
				}

                if (r.Code == ExcelOperate.Biff.Type.BOF.Value)
				{
					bofs++;
				}
			}
...
//		}
		...
		/// </deprecated>
        public virtual ExcelOperate.Format.CellFormat getColumnFormat(int col)
		{
