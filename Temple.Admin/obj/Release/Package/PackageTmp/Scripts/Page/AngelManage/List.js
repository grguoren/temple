...
var AngelList = avalon.define('AngelList', function (vm) {
    vm.array = [];
    vm.changeList = [];
    vm.changeAll = false;
    vm.List_Page = 1;
    ...
    vm.getList = function (page) {
        ...
        $.ajax({
            type: 'POST',
            url: Config.WebUrl + 'Ajax_Angel/GetAngelList',
    ...
    //    $.ajax({
    //        type: 'POST',
    //        url: Config.WebUrl + 'Ajax_Angel/UpdateNewsTypeSort',
    ...
    //}
    //-------------------------删除法會
    vm.DeleteAngelFun = function (id) {
        if (window.confirm('你確定要刪除嗎？')) {
            $.ajax({
                type: 'POST',
                url: Config.WebUrl + 'Ajax_Angel/DeleteAngel',
                data: { id: id },
                timeout: 200000,
                success: function (ajaxData) {
                    if (ajaxData === "True") {
    ...
    vm.DeleteNewsTypeAllFun = function () {
        if (vm.changeList.length > 0) {
            if (window.confirm('你確定要刪除選定項嗎？')) {
                ...
                $.ajax({
                    type: 'POST',
                    url: Config.WebUrl + 'Ajax_Angel/DeleteAllNewsType',
                    data: { list: str },
                    timeout: 200000,
                    success: function (ajaxData) {
                        if (!ajaxData.Status) {
