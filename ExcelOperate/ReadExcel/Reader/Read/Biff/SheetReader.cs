...
using ExcelOperate.Biff.Drawing;
	...
	sealed class SheetReader 
	{
		...
		private void  addCell(Cell cell)
		{
			...
			if (cell.Row < numRows && cell.Column < numCols)
			{
				if (cells[cell.Row][cell.Column] != null)
				{
                    ...
                    ExcelOperate.ReadExcel.CellReferenceHelper.getCellReference(cell.Column, cell.Row, sb);
		...
		internal void  read()
		{
			...
			while (cont)
			{
                ...
                if (r.Type == ExcelOperate.Biff.Type.UNKNOWN && r.Code == 0)
				{
					...
					if (r.Length == 0xa)
					{
                        ...
                        r.Type = (ExcelOperate.Biff.Type.DIMENSION);
					}
					else
					{
                ...
                if (r.Type == ExcelOperate.Biff.Type.DIMENSION)
				{
                ...
                else if (r.Type == ExcelOperate.Biff.Type.LABELSST)
				{
					LabelSSTRecord label = new LabelSSTRecord(r, sharedStrings, formattingRecords, sheet);
					addCell(label);
				}
                else if (r.Type == ExcelOperate.Biff.Type.RK || r.Type == ExcelOperate.Biff.Type.RK2)
				{
                ...
                else if (r.Type == ExcelOperate.Biff.Type.HLINK)
				{
					HyperlinkRecord hr = new HyperlinkRecord(r, sheet, workbookSettings);
					hyperlinks.Add(hr);
				}
                else if (r.Type == ExcelOperate.Biff.Type.MERGEDCELLS)
				{
                ...
                else if (r.Type == ExcelOperate.Biff.Type.MULRK)
				{
                ...
                else if (r.Type == ExcelOperate.Biff.Type.NUMBER)
				{
                ...
                else if (r.Type == ExcelOperate.Biff.Type.BOOLERR)
				{
                ...
                else if (r.Type == ExcelOperate.Biff.Type.PANE)
				{
                ...
                else if (r.Type == ExcelOperate.Biff.Type.CONTINUE)
				{
					;
				}
                else if (r.Type == ExcelOperate.Biff.Type.NOTE)
				{
					;
				}
                else if (r.Type == ExcelOperate.Biff.Type.ARRAY)
				{
					;
				}
                else if (r.Type == ExcelOperate.Biff.Type.PROTECT)
				{
					ProtectRecord pr = new ProtectRecord(r);
					settings.Protected=(pr.IsProtected());
				}
                else if (r.Type == ExcelOperate.Biff.Type.SHAREDFORMULA)
				{
                ...
                else if (r.Type == ExcelOperate.Biff.Type.FORMULA || r.Type == ExcelOperate.Biff.Type.FORMULA2)
				{
                ...
                else if (r.Type == ExcelOperate.Biff.Type.LABEL)
				{
					LabelRecord lr = null;
                ...
                else if (r.Type == ExcelOperate.Biff.Type.RSTRING)
				{
                ...
                else if (r.Type == ExcelOperate.Biff.Type.NAME)
				{
					;
				}
                else if (r.Type == ExcelOperate.Biff.Type.PASSWORD)
				{
					PasswordRecord pr = new PasswordRecord(r);
					settings.PasswordHash=(pr.PasswordHash);
				}
                else if (r.Type == ExcelOperate.Biff.Type.ROW)
				{
                ...
                else if (r.Type == ExcelOperate.Biff.Type.BLANK)
				{
					BlankCell bc = new BlankCell(r, formattingRecords, sheet);
					addCell(bc);
				}
                else if (r.Type == ExcelOperate.Biff.Type.MULBLANK)
				{
                ...
                else if (r.Type == ExcelOperate.Biff.Type.SCL)
				{
					SCLRecord scl = new SCLRecord(r);
					settings.ZoomFactor = (scl.ZoomFactor);
				}
                else if (r.Type == ExcelOperate.Biff.Type.COLINFO)
				{
					ColumnInfoRecord cir = new ColumnInfoRecord(r);
					columnInfosArray.Add(cir);
				}
                else if (r.Type == ExcelOperate.Biff.Type.HEADER)
				{
                    ...
                    ExcelOperate.ReadExcel.HeaderFooter header = new ExcelOperate.ReadExcel.HeaderFooter(hr.Header);
					settings.Header = (header);
				}
                else if (r.Type == ExcelOperate.Biff.Type.FOOTER)
				{
                    ...
                    ExcelOperate.ReadExcel.HeaderFooter footer = new ExcelOperate.ReadExcel.HeaderFooter(fr.Footer);
					settings.Footer=(footer);
				}
                else if (r.Type == ExcelOperate.Biff.Type.SETUP)
				{
                ...
                else if (r.Type == ExcelOperate.Biff.Type.WSBOOL)
				{
					workspaceOptions = new WorkspaceInformationRecord(r);
				}
                else if (r.Type == ExcelOperate.Biff.Type.DEFCOLWIDTH)
				{
					DefaultColumnWidthRecord dcwr = new DefaultColumnWidthRecord(r);
					settings.DefaultColumnWidth = (dcwr.Width);
				}
                else if (r.Type == ExcelOperate.Biff.Type.DEFAULTROWHEIGHT)
				{
                ...
                else if (r.Type == ExcelOperate.Biff.Type.PLS)
				{
					plsRecord = new PLSRecord(r);
				}
                else if (r.Type == ExcelOperate.Biff.Type.OBJ)
				{
                ...
                else if (r.Type == ExcelOperate.Biff.Type.MSODRAWING)
				{
					msoRecord = new MsoDrawingRecord(r);
				}
                else if (r.Type == ExcelOperate.Biff.Type.BOF)
				{
                    ...
                    while (r2.Code != ExcelOperate.Biff.Type.EOF.Value)
					{
						r2 = excelFile.next();
					}
                ...
                else if (r.Type == ExcelOperate.Biff.Type.EOF)
				{
					cont = false;
				}
			}
