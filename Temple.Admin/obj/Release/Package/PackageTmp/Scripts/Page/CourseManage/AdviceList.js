var MessageList = avalon.define('messageList', function (vm) {
    ...
    vm.getList = function (page) {
        ...
        $.ajax({
            type: 'POST',
            url: Config.WebUrl + 'Ajax_Message/GetMessageList',
    ...
    vm.DeleteMessageFun = function (id) {
        if (window.confirm('你确定要删除吗？')) {
            $.ajax({
                type: 'POST',
                url: Config.WebUrl + 'Ajax_Message/DeleteMessage',
                data: { id: id },
                timeout: 200000,
                success: function (ajaxData) {
                    if (ajaxData === "True") {
    ...
    vm.DeleteMessageAllFun = function () {
        if (vm.changeList.length > 0) {
            if (window.confirm('你确定要删除选定项吗？')) {
                ...
                $.ajax({
                    type: 'POST',
                    url: Config.WebUrl + 'Ajax_Message/DeleteMessageAll',
                    data: { list: str },
                    timeout: 200000,
                    success: function (ajaxData) {
                        if (!ajaxData.Status) {
    ...
    vm.VerifyMessageAllFun = function (status) {
        if (vm.changeList.length > 0) {
            if (window.confirm('你确定要更改选定项的审核状态吗？')) {
                ...
                $.ajax({
                    type: 'POST',
                    url: Config.WebUrl + 'Ajax_Message/VerifyAllMessage',
    ...
    vm.VerifyMessageFun = function (id, status) {
        if (window.confirm('你确定要更改审核状态吗？')) {
            $.ajax({
                type: 'POST',
                url: Config.WebUrl + 'Ajax_Message/VerifyMessage',
    ...
    vm.UpdateAddTimeFun = function (id, addtime) {
        if (window.confirm('你确定要更改时间吗？')) {
            $.ajax({
                type: 'POST',
                url: Config.WebUrl + 'Ajax_Message/UpdateMessageAddTime',
