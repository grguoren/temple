...
var AboutTypeList = avalon.define('aboutList', function (vm) {
    vm.array = [];
    vm.changeList = [];
    vm.changeAll = false;
    vm.List_Page = 1;
    ...
    vm.EditSort = function (id, Sort) {
        ...
        $.ajax({
            type: 'POST',
            url: Config.WebUrl + 'Ajax_About/UpdateAboutTypeSort',
    ...
    vm.DeleteAboutTypeFun = function (id) {
        if (window.confirm('你确定要删除吗？')) {
            $.ajax({
                type: 'POST',
                url: Config.WebUrl + 'Ajax_About/DeleteAboutType',
                data: { id: id },
                timeout: 200000,
                success: function (ajaxData) {
                    if (ajaxData === "True") {
    ...
    vm.DeleteAboutTypeAllFun = function () {
        if (vm.changeList.length > 0) {
            if (window.confirm('你确定要删除选定项吗？')) {
                ...
                $.ajax({
                    type: 'POST',
                    url: Config.WebUrl + 'Ajax_About/DeleteAllAboutType',
                    data: { list: str },
                    timeout: 200000,
                    success: function (ajaxData) {
                        if (!ajaxData.Status) {
