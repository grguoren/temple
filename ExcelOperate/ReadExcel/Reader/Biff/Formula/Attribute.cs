...
namespace ExcelOperate.Biff.Formula
{
	...
	class Attribute:Operator, ParsedThing
	{
		...
		public Attribute(StringFunction sf, WorkbookSettings ws)
		{
            ...
            if (sf.getFunction(settings) == ExcelOperate.Biff.Formula.Function.SUM)
			{
				options |= sumMask;
			}
            else if (sf.getFunction(settings) == ExcelOperate.Biff.Formula.Function.IF)
			{
				options |= ifMask;
			}
		}
		...
		public override void  getString(System.Text.StringBuilder buf)
		{
			...
			{
				ParseItem[] operands = getOperands();
                buf.Append(ExcelOperate.Biff.Formula.Function.SUM.getName(settings));
				buf.Append('(');
			...
			{
                buf.Append(ExcelOperate.Biff.Formula.Function.IF.getName(settings));
				buf.Append('(');
