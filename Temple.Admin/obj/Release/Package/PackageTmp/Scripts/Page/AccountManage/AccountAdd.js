...
function AddData_Ajax() {
    if (InitValidatorDataFun()) {
        $.ajax({
            type: 'POST',
            url: Config.WebUrl + 'Ajax_Account/AddAccount',
            data: {
            ...
            success: function (ajaxData) {
                if (ajaxData === "True") {
                    alert('添加成功');
                    location.href = Config.WebUrl + "AccountManage/AccountList";
                }
            },
            dataType: 'text'
        });
...
function UpdateData_Ajax() {
    if (InitValidatorDataFun()) {
        $.ajax({
            type: 'POST',
            url: Config.WebUrl + 'Ajax_Account/UpdateAccount',
            data: {
                name: DataAddOrUpdate.NewModel.Name,
                code: DataAddOrUpdate.NewModel.Code,
                remark: DataAddOrUpdate.NewModel.Remark,
...
function GetDataById_Ajax() {
    $.ajax({
        type: 'POST',
        url: Config.WebUrl + 'Ajax_Account/GetAccountById',
