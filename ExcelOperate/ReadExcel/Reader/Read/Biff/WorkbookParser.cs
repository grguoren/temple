	...
	public class WorkbookParser:Workbook, ExternalSheet, WorkbookMethods
	{
		...
		protected internal override void  parse()
		{
			...
			while (bofs == 1)
			{
                ...
                if (r.Type == ExcelOperate.Biff.Type.SST)
				{
					continueRecords.Clear();
					Record nextrec = excelFile.peek();
                    while (nextrec.Type == ExcelOperate.Biff.Type.CONTINUE)
					{
                ...
                else if (r.Type == ExcelOperate.Biff.Type.FILEPASS)
				{
                ...
                else if (r.Type == ExcelOperate.Biff.Type.FONT)
				{
					FontRecord fr = null;
                ...
                else if (r.Type == ExcelOperate.Biff.Type.PALETTE)
				{
                    ExcelOperate.Biff.PaletteRecord palette = new ExcelOperate.Biff.PaletteRecord(r);
					formattingRecords.Palette = palette;
				}
                else if (r.Type == ExcelOperate.Biff.Type.NINETEENFOUR)
				{
					NineteenFourRecord nr = new NineteenFourRecord(r);
					nineteenFour = nr.is1904();
				}
                else if (r.Type == ExcelOperate.Biff.Type.FORMAT)
				{
                ...
                else if (r.Type == ExcelOperate.Biff.Type.XF)
				{
					XFRecord xfr = null;
					if (bof.isBiff8())
					{
                ...
                else if (r.Type == ExcelOperate.Biff.Type.BOUNDSHEET)
				{
					BoundsheetRecord br = null;
                ...
                else if (r.Type == ExcelOperate.Biff.Type.EXTERNSHEET)
				{
					if (bof.isBiff8())
					{
                ...
                else if (r.Type == ExcelOperate.Biff.Type.CODEPAGE)
				{
					CodepageRecord cr = new CodepageRecord(r);
					settings.CharacterSet = cr.CharacterSet;
				}
                else if (r.Type == ExcelOperate.Biff.Type.SUPBOOK)
				{
                ...
                else if (r.Type == ExcelOperate.Biff.Type.MSODRAWINGGROUP)
				{
			...
			if (excelFile.hasNext())
			{
                ...
                if (r.Type == ExcelOperate.Biff.Type.BOF)
				{
					bof = new BOFRecord(r);
				}
			}
			...
			while (bof != null && NumberOfSheets < boundsheets.Count)
			{
				...
				else
				{
                    ...
                    while (excelFile.hasNext() && r.Type != ExcelOperate.Biff.Type.EOF)
					{
						r = excelFile.next();
					}
				}
