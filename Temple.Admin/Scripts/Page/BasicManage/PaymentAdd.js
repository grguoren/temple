...
function AddData_Ajax() {
    if (InitValidatorDataFun()) {
        $.ajax({
            type: 'POST',
            url: Config.WebUrl + 'Ajax_Payment/AddPayment',
            data: {
            ...
            success: function (ajaxData) {
                if (ajaxData === "True") {
                    alert('添加成功');
                    location.href = Config.WebUrl + "BasicManage/Payment";
                }
            },
            dataType: 'text'
        });
...
function UpdateData_Ajax() {
    if (InitValidatorDataFun()) {
        $.ajax({
            type: 'POST',
            url: Config.WebUrl + 'Ajax_Payment/UpdatePayment',
            data: {
                name: DataAddOrUpdate.NewModel.Name,
                code: DataAddOrUpdate.NewModel.Code,
                remark: DataAddOrUpdate.NewModel.Remark,
...
function GetDataById_Ajax() {
    $.ajax({
        type: 'POST',
        url: Config.WebUrl + 'Ajax_Payment/GetPaymentById',
