...
namespace ExcelOperate.Read.Biff
{
	...
	public class File 
	{
		...
		public File(System.IO.Stream stream, WorkbookSettings ws)
		{
            ...
            int bytesRead = ExcelOperate.Utils.File.ReadInput(stream, ref d, 0, d.Length);
			int pos = bytesRead;
			...
			while (bytesRead != - 1)
			{
                ...
                bytesRead = ExcelOperate.Utils.File.ReadInput(stream, ref d, pos, d.Length - pos);
				pos += bytesRead;
		...
		private void  moveToFirstBof()
		{
			...
			while (!bofFound)
			{
                ...
                if (code == ExcelOperate.Biff.Type.BOF.Value)
				{
					bofFound = true;
				}
				else
