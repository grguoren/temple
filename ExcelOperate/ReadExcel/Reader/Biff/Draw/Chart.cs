...
namespace ExcelOperate.Biff.Drawing
{
	...
	public class Chart : ByteData, EscherStream
	{
		...
		public virtual void  rationalize(IndexMapping xfMapping, IndexMapping fontMapping, IndexMapping formatMapping)
		{
			...
			int code = 0;
			int length = 0;
            ExcelOperate.Biff.Type type = null;
			while (pos < data.Length)
			{
				code = IntegerHelper.getInt(data[pos], data[pos + 1]);
				length = IntegerHelper.getInt(data[pos + 2], data[pos + 3]);

                type = ExcelOperate.Biff.Type.getType(code);

                if (type == ExcelOperate.Biff.Type.FONTX)
				{
                ...
                else if (type == ExcelOperate.Biff.Type.FBI)
				{
                ...
                else if (type == ExcelOperate.Biff.Type.IFMT)
				{
