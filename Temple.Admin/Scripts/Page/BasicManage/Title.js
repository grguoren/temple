...
var TitleList = avalon.define('TitleList', function (vm) {
    vm.array = [];
    vm.changeList = [];
    vm.changeAll = false;
    vm.List_Page = 1;
    ...
    vm.getList = function (page) {
        ...
        $.ajax({
            type: 'POST',
            url: Config.WebUrl + 'Ajax_Title/GetTitleList',
    ...
    //}
    //-------------------------删除稱謂
    vm.DeleteTitleFun = function (id) {
        if (window.confirm('你確定要刪除嗎？')) {
            $.ajax({
                type: 'POST',
                url: Config.WebUrl + 'Ajax_Title/DeleteTitle',
                data: { id: id },
                timeout: 200000,
                success: function (ajaxData) {
                    if (ajaxData === "True") {
