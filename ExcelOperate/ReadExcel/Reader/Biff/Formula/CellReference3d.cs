...
namespace ExcelOperate.Biff.Formula
{
	...
	class CellReference3d:Operand, ParsedThing
	{
		...
		public CellReference3d(string s, ExternalSheet w)
		{
			...
			column = ExcelOperate.Biff.CellReferenceHelper.getColumn(cellString);
			row = ExcelOperate.Biff.CellReferenceHelper.getRow(cellString);
		...
		{
			ExcelOperate.Biff.CellReferenceHelper.getCellReference(sheet, column, !columnRelative, row, !rowRelative, workbook, buf);
		}
