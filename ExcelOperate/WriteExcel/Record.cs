...
namespace ExcelOperate.WriteExcel
{
    internal class Record
    {
        ...
        public static Record Empty = new Record(ExcelOperate.WriteExcel.RID.Empty, new byte[0]);
        ...
        {
            _rid = ExcelOperate.WriteExcel.RID.ByteArray(rid);
            int offset = 0;
            ...
            while (bytesRemaining > 0)
            {
                ...
                if (continueIndex == -1)
                    ...
                    _continues.Add(new Record(ExcelOperate.WriteExcel.RID.CONTINUE, data.Get(offset, bytesToAppend)));
        ...
        internal static Bytes GetBytes(byte[] rid, Bytes data)
        {
            ...
            do
            {
                ...
                {
                    record.Append(ExcelOperate.WriteExcel.RID.CONTINUE);
                    record.Append(BitConverter.GetBytes(length));
                    record.Append(data.Get(offset, length));
                }
        ...
        {
            return (_rid == ExcelOperate.WriteExcel.RID.RK ||
                    _rid == ExcelOperate.WriteExcel.RID.NUMBER ||
                    _rid == ExcelOperate.WriteExcel.RID.LABEL ||
                    _rid == ExcelOperate.WriteExcel.RID.LABELSST ||
                    _rid == ExcelOperate.WriteExcel.RID.MULBLANK ||
                    _rid == ExcelOperate.WriteExcel.RID.MULRK ||
                    _rid == ExcelOperate.WriteExcel.RID.FORMULA);
        }
        ...
        public static List<Record> GetAll(Bytes stream)
        {
            ...
            {
                byte[] rid = ExcelOperate.WriteExcel.RID.ByteArray(stream.Get(i, 2).ByteArray);
                Bytes data = new Bytes();
                if (rid == ExcelOperate.WriteExcel.RID.Empty)
                    break;
                int length = BitConverter.ToUInt16(stream.Get(i + 2, 2).ByteArray, 0);
                data = stream.Get(i + 4, length);
                Record record = new Record(rid, data);
                i += (4 + length);
                if (rid == ExcelOperate.WriteExcel.RID.CONTINUE)
                {
