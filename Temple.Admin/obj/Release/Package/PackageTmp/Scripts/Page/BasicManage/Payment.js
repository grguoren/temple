...
var PaymentList = avalon.define('PaymentList', function (vm) {
    vm.array = [];
    vm.changeList = [];
    vm.changeAll = false;
    vm.List_Page = 1;
    ...
    vm.getList = function (page) {
        ...
        $.ajax({
            type: 'POST',
            url: Config.WebUrl + 'Ajax_Payment/GetPaymentList',
    ...
    //}
    //-------------------------删除付款方式
    vm.DeletePaymentFun = function (id) {
        if (window.confirm('你確定要刪除嗎？')) {
            $.ajax({
                type: 'POST',
                url: Config.WebUrl + 'Ajax_Payment/DeletePayment',
                data: { id: id },
                timeout: 200000,
                success: function (ajaxData) {
                    if (ajaxData === "True") {
